...
> **轻量级、跨平台的实时歌曲播放进度同步与状态监测系统**。支持任意音乐 App 播放进度的秒级同步监测、歌词同步滚动、专辑封面提取，并在播放暂停时自动展示艺术家人像轮播，同时集成社交动态微信推送。
> 
...
- 🔔 **全自动动态监控**：轻量级后台爬虫（支持部署于 OpenWrt 路由器等设备）监控微博/B站/抖音动态，发现新内容时通过 WxPusher 实时推送到微信
- 🔄 **滚动图库更新**：每次爬虫运行时自动更新最新的 100 张人像到云端，旧图自动滚出
...
| 必需 | 说明 |
|------|------|
| [Cloudflare 账号](https://dash.cloudflare.com/sign-up)（免费） | 用于部署 Worker 和 KV 数据库 |
| 一个域名 | 绑定到 Cloudflare 的域名（购买后托管到 CF 即可） |
| Python 3.8+ | 用于运行部署脚本和路由器爬虫 |

| 可选 | 说明 |
|------|------|
| OpenWrt 路由器 | 用于 7×24 小时后台运行自动爬虫 |
| [WxPusher 账号](https://wxpusher.zjiecode.com)（免费） | 用于微信动态推送通知 |
| 安卓设备 + SLS 音乐记录器 | 实时上报播放状态 |
...
# WxPusher 推送配置（从 wxpusher.zjiecode.com 获取）
