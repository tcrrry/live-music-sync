...
async function forwardToRouter(bodyText) {
  ...
  try {
    ...
    const resp = await fetch("https://router.yourdomain.com/cgi-bin/music_status", {
...
async function handleRequest(request) {
  ...
  if (url.pathname === "/api/status") {
    ...
    if (request.method === "GET") {
      ...
      // Check if debug parameter is requested
      if (url.searchParams.get("debug") === "1") {
        const raw = await MUSIC_KV.get("raw_post_debug");
        return new Response(raw || JSON.stringify({ error: "No debug data yet" }), {
      ...
      try {
        ...
        if (data) {
          // Temporarily save the raw payload from the phone to KV for debugging
          await MUSIC_KV.put("raw_post_debug", JSON.stringify(data));
        ...
        if (isIncomingTrash) {
          ...
          try {
            ...
            const rResp = await fetch("https://router.yourdomain.com/cgi-bin/music_status").catch(() => null);
